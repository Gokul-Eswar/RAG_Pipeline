...
class TestEventIngestHandler:
    ...
    def test_ingest_event_valid(self, auth_client, sample_event):
    ...
    def test_ingest_event_missing_text(self, auth_client):
        """Test ingestion validation."""
        response = auth_client.post("/ingest/", json={"id": "test", "text": ""})
...
class TestVectorMemoryHandler:
    ...
    def test_upsert_vectors(self, auth_client, sample_vector):
        """Test vector upsert."""
        payload = {
            "collection": "test_vectors",
            "vectors": [sample_vector]
        }
        response = auth_client.post("/memory/upsert", json=payload)
        assert response.status_code in [200, 500]
    
    def test_search_vectors(self, auth_client):
        """Test vector search."""
        payload = {
            "query_vector": [0.1] * 384,
            "limit": 5
        }
        response = auth_client.post("/memory/search", json=payload)
        assert response.status_code in [200, 500]
    
    def test_collection_info(self, auth_client):
        """Test collection info endpoint."""
        payload = {"collection": "test_vectors"}
        response = auth_client.post("/memory/collection/info", json=payload)
        assert response.status_code in [200, 500]
...
class TestGraphMemoryHandler:
    ...
    def test_create_node(self, auth_client, sample_node):
        """Test node creation."""
        response = auth_client.post("/graph/node", json=sample_node)
        assert response.status_code in [200, 500]
    ...
    def test_create_relationship(self, auth_client):
        ...
        response = auth_client.post("/graph/relationship", json=payload)
        assert response.status_code in [200, 500]
    
    def test_find_node(self, auth_client):
        """Test node search."""
        payload = {
            "label": "TestEntity",
            "properties": {"name": "nonexistent"}
        }
        response = auth_client.post("/graph/node/find", json=payload)
        assert response.status_code in [200, 404, 500]
    
    def test_delete_node(self, auth_client):
        """Test node deletion."""
        response = auth_client.delete("/graph/node/999")
        assert response.status_code in [200, 404, 500]
